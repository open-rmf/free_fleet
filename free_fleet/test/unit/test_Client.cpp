...
public:
  ...
  void received_dock_request(
    ...
    free_fleet::CommandId command_id,
    const std::string& dock_name)
  {
    if (dock_request_callback)
    {
      free_fleet::messages::DockRequest request(
        robot_name, command_id, dock_name);
      dock_request_callback(request);
    }
  }

  void received_pause_request(
    const std::string& robot_name, free_fleet::CommandId command_id)
  {
    if (pause_request_callback)
    {
      free_fleet::messages::PauseRequest request(robot_name, command_id);
    ...
    const std::string& robot_name, free_fleet::CommandId command_id)
  {
    ...
    {
      free_fleet::messages::ResumeRequest request(robot_name, command_id);
