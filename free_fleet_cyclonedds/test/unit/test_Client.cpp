...
SCENARIO("Single client with mock subscribers and publishers")
{
  ...
  CHECK(std::string(received_states[0]->task_id) == current_task_id);
  ...
  mode_req_msg->task_id = dds_string_alloc_and_copy("1");
  ...
  CHECK(mode_request.value().task_id == "1");
  ...
  nav_req_msg->task_id = dds_string_alloc_and_copy("2");
  ...
  CHECK(nav_request.value().task_id == "2");
  ...
  reloc_req_msg->task_id = dds_string_alloc_and_copy("3");
  ...
  CHECK(reloc_request.value().task_id == "3");
